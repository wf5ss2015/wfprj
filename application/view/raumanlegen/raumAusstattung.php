...
<?php
	...
	if ($this->ausstattung_list){
		foreach($this->ausstattung_list as $key => $value){
			...
			echo '<input type="checkbox" name="'; echo htmlentities($value->ausstattung_bezeichnung); echo '" value="'; echo htmlentities($value->ausstattung_ID); echo '"> Anzahl:<input class="tf" type = "text" name = "';echo htmlentities($value->ausstattung_bezeichnung);echo 'Anzahl"/><br>'; 
